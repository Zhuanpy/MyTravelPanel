...
def manage_visas():
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return jsonify({
            'success': True,
            'html': render_template('visas/签证文档管理/visa_document_table.html', documents=documents)
        })

    return render_template('visas/签证文档管理/visa_document.html', 
                         documents=documents, 
...
def fix_share_documents_route(visa_type):
    """修复指定签证类型的SHARE文档问题"""
    try:
        from ..utils.visa_debug import fix_share_documents
...
def debug_visa_documents_route(visa_type):
    """调试指定签证类型的文档数据"""
    try:
        from ..utils.visa_debug import debug_visa_documents
...
def check_all_share_records():
    """检查所有签证类型的SHARE记录状态"""
    try:
        from ..utils.visa_debug import check_all_visa_types
