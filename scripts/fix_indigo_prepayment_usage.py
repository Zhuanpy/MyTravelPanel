# -*- coding: utf-8 -*-
"""
修复 INDIGO AIRLINE SINGAPORE 预付账款使用记录
...
确认执行: python scripts/fix_indigo_prepayment_usage.py --confirm
"""
...
def main():
    parser = argparse.ArgumentParser(description='修复 INDIGO 预付账款使用记录')
    ...
    with app.app_context():
        print("=" * 70)
        print("  修复 INDIGO AIRLINE SINGAPORE 预付账款使用记录")
        print("=" * 70)
        ...
        # 1. 查找 INDIGO 供应商
        supplier = CustomerCompany.query.filter_by(company_name='INDIGO AIRLINE SINGAPORE').first()
        if not supplier:
            print("\n错误: 未找到供应商 'INDIGO AIRLINE SINGAPORE'")
            return
        ...
        if not dry_run:
            ...
            print(f"\n确认修复请添加 --confirm 参数:")
            print(f"  python scripts/fix_indigo_prepayment_usage.py --confirm")
