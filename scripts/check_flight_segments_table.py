#!/usr/bin/env python3
"""
检查 project_flight_segments 表结构的脚本
"""
...
def check_flight_segments_table():
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            flight_ref_id_exists = any(col['Field'] == 'flight_ref_id' for col in columns)
            
            if ref_id_exists:
                print("\n✅ ref_id 字段存在")
            elif flight_ref_id_exists:
                print("\n❌ 字段名为 flight_ref_id，需要改为 ref_id")
                return False
            else:
                print("\n❌ 既没有 ref_id 也没有 flight_ref_id 字段")
                return False
            ...
            cursor.execute("""
                ...
                AND TABLE_NAME = 'project_flight_segments'
...
if __name__ == "__main__":
    print("开始检查 project_flight_segments 表结构...")
    success = check_flight_segments_table()
    if success:
        print("\n✅ 表结构检查完成")
    else:
        print("\n❌ 表结构检查失败")
