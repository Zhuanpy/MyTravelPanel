# -*- coding: utf-8 -*-
"""
修复 AIR INDIA EXPRESS 预付账款使用记录

supplier_id = 228

运行方式: python scripts/fix_airindia_prepayment_usage.py
确认执行: python scripts/fix_airindia_prepayment_usage.py --confirm
"""
...
# AIR INDIA EXPRESS 的 supplier_id
SUPPLIER_ID = 228


def main():
    parser = argparse.ArgumentParser(description='修复 AIR INDIA EXPRESS 预付账款使用记录')
    ...
    with app.app_context():
        print("=" * 70)
        print("  修复 AIR INDIA EXPRESS 预付账款使用记录")
        print("=" * 70)
            ...
            while amount_to_allocate > 0 and prepayment_index < len(prepayments):
                ...
                if not dry_run:
                    ...
                    usage = PrepaymentUsage(
                        ...
                        description=f'FIFO {eo.eo_number}',
                        status='confirmed',
        ...
        if not dry_run:
            ...
            print(f"\n确认修复请添加 --confirm 参数:")
            print(f"  python scripts/fix_airindia_prepayment_usage.py --confirm")
