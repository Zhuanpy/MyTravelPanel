...
def add_product_type_field():
    ...
    with app.app_context():
        ...
        try:
            # 添加product_type字段
            sql = "ALTER TABLE travelproducts ADD COLUMN product_type VARCHAR(50) NULL COMMENT '产品类型：跟团游/自由行/定制游'"
            print(f"执行SQL: {sql}")
            cursor.execute(sql)
            print("✓ product_type字段添加成功")
