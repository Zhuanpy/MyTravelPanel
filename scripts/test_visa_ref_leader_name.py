...
def test_visa_ref_leader_name():
    """测试REF创建时负责人姓名字段的填充"""
    app = create_app()
    
    with app.app_context():
        try:
            print("开始测试REF创建时负责人姓名字段的填充...")
            ...
            # 验证联系人姓名
            print(f"\n验证联系人姓名:")
            print(f"  签证项目联系人: {test_project.contact_name}")
            print(f"  REF联系人: {ref_verify.contact_name}")
            expected_contact = test_project.contact_name or test_project.applicant_name
            print(f"  预期联系人: {expected_contact}")
            print(f"  联系人姓名匹配: {ref_verify.contact_name == expected_contact}")
