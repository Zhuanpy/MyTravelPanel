...
-- 步骤3：添加新的外键约束（使用唯一的约束名称）

-- 修复 Todo 表的外键约束
ALTER TABLE todos 
ADD CONSTRAINT todos_user_id_cascade_fk 
FOREIGN KEY (user_id) REFERENCES auth_users(id) ON DELETE CASCADE;

-- 修复 InvitationCode 表的外键约束
ALTER TABLE invitation_codes 
ADD CONSTRAINT invitation_codes_created_by_null_fk 
FOREIGN KEY (created_by) REFERENCES auth_users(id) ON DELETE SET NULL;

ALTER TABLE invitation_codes 
ADD CONSTRAINT invitation_codes_used_by_null_fk 
FOREIGN KEY (used_by) REFERENCES auth_users(id) ON DELETE SET NULL;

-- 确保 UserProfile 表的外键约束正确
ALTER TABLE user_profiles 
ADD CONSTRAINT user_profiles_user_id_cascade_fk 
FOREIGN KEY (user_id) REFERENCES auth_users(id) ON DELETE CASCADE;
