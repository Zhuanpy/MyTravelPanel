...
def uob_bank():
    ...
    if filter_diff_nonzero:
        ...
        print(f"UOB差额非0筛选: 共 {total} 条记录符合条件")
    else:
        # 获取分页数据
...
@uob_blue.route('/uob_batch_unlock', methods=['POST'])
@csrf.exempt
@login_required
@staff_only
def uob_batch_unlock():
    """批量解锁UOB银行交易记录"""
    try:
        data = request.get_json()
...
@uob_blue.route('/uob_unmatch_transaction/<int:transaction_id>', methods=['POST'])
@csrf.exempt
@login_required
@staff_only
def uob_unmatch_transaction(transaction_id):
    """解除UOB银行交易的匹配关系"""
    try:
        transaction = BankTransaction.query.get(transaction_id)
...
@uob_blue.route('/uob_reconcile_transaction/<int:transaction_id>', methods=['POST'])
@csrf.exempt
@login_required
@staff_only
def uob_reconcile_transaction(transaction_id):
    """标记UOB银行交易为已核对"""
    try:
        transaction = BankTransaction.query.get(transaction_id)
