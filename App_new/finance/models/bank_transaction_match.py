...
class BankTransactionMatch(db.Model):
    ...
    match_type = db.Column(db.Enum('receipt', 'eo', 'payment', 'prepayment', 'loan_borrow', 'loan_repay', 'operating_expense'), nullable=False,
                          comment='匹配类型：receipt=收款，eo=EO，payment=付款，prepayment=预付款，loan_borrow=股东借款，loan_repay=股东还款，operating_expense=运营费用')
